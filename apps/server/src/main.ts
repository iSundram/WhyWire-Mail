...
import { getWhyWireAgent, getWhyWireDB, verifyToken } from './lib/server-utils';
import { SyncThreadsWorkflow } from './workflows/sync-threads-workflow';
import { ShardRegistry, WhyWireAgent, WhyWireDriver } from './routes/agent';
...
import { WhyWireMCP } from './routes/agent/mcp';
...
class WhyWireDB extends DurableObject<ZeroEnv> {
    ...
    if (c.req.header('Authorization') && !session?.user) {
      ...
      if (token) {
        try {
          ...
          if (userId) {
            const db = await getWhyWireDB(userId);
...
const app = new Hono<HonoContext>()
  ...
  .mount(
    ...
    async (request, env, ctx) => {
      ...
      return WhyWireMCP.serveSSE('/sse', { binding: 'WHYWIRE_MCP' }).fetch(request, env, ctx);
    },
  ...
  .mount(
    ...
    async (request, env, ctx) => {
      ...
      return WhyWireMCP.serve('/mcp', { binding: 'WHYWIRE_MCP' }).fetch(request, env, ctx);
    },
    { replaceRequest: false },
  )
  .route('/api', api)
...
export default class Entry extends WorkerEntrypoint<ZeroEnv> {
      ...
      case batch.queue.startsWith('send-email-queue'): {
        await Promise.all(
          batch.messages.map(async (msg: any) => {
            ...
            const agent = await getWhyWireAgent(connectionId, this.ctx);
            try {
  ...
  private async processExpiredSubscriptions() {
    ...
    //     try {
    //       const { stub: agent } = await getWhyWireAgent(connectionId, this.ctx);
