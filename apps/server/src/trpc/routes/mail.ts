...
export const mailRouter = router({
  suggestRecipients: activeDriverProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const { stub: agent } = await getWhyWireAgent(activeConnection.id, executionCtx);
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (threadsResponse.threads.length === 0 && folder === FOLDERS.INBOX && !q) {
        ...
        if (now - lastResync > RESYNC_COOLDOWN_MS) {
          ...
          getWhyWireAgent(activeConnection.id, executionCtx)
            .then((_agent) => {
  ...
  send: activeDriverProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const executionCtx = getContext<HonoContext>().executionCtx;
      const agent = await getWhyWireAgent(activeConnection.id, executionCtx);
  ...
  delete: activeDriverProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const { activeConnection } = ctx;
      const executionCtx = getContext<HonoContext>().executionCtx;
      const { exec, stub } = await getWhyWireAgent(activeConnection.id, executionCtx);
  ...
  getRawEmail: activeDriverProcedure
    ...
    .query(async ({ input, ctx }) => {
      const { activeConnection } = ctx;
      const { stub: agent } = await getWhyWireAgent(activeConnection.id);
