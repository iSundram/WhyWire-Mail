...
export const getWhyWireDB = async (userId: string) => {
  const stub = env.WHYWIRE_DB.get(env.WHYWIRE_DB.idFromName(userId));
...
const getShardClient = async (connectionId: string, shardId: string) => {
  const shardClient = createClient({
    doNamespace: env.WHYWIRE_DRIVER,
    ctx: new MockExecutionContext(),
...
export const modifyThreadLabelsInDB = async (
  ...
  const agent = await getWhyWireSocketAgent(connectionId);
...
export const getWhyWireAgent = async (connectionId: string, executionCtx?: ExecutionContext) => {
  if (!executionCtx) {
...
export const getWhyWireAgentFromShard = async (connectionId: string, shardId: string) => {
  const agent = await getShardClient(connectionId, shardId);
  return agent;
};
...
export const forceReSync = async (connectionId: string) => {
  ...
  const agent = await getWhyWireAgent(connectionId);
...
export const reSyncThread = async (connectionId: string, threadId: string) => {
  try {
    ...
    const agent = await getWhyWireAgentFromShard(connectionId, shardId);
...
export const getWhyWireSocketAgent = async (connectionId: string) => {
  const stub = env.WHYWIRE_AGENT.get(env.WHYWIRE_AGENT.idFromName(connectionId));
  return stub;
};
...
export const getActiveConnection = async () => {
  ...
  const db = await getWhyWireDB(sessionUser.id);
