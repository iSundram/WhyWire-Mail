...
import { getWhyWireDB, resetConnection } from './server-utils';
...
const connectionHandlerHook = async (account: Account) => {
  ...
  const db = await getWhyWireDB(account.userId);
    ...
    user: {
      deleteUser: {
        ...
        beforeDelete: async (user, request) => {
          ...
          const db = await getWhyWireDB(user.id);
      ...
      after: createAuthMiddleware(async (ctx) => {
        ...
        if (ctx.path.startsWith('/sign-up')) {
          ...
          if (newSession) {
            ...
            const db = await getWhyWireDB(newSession.user.id);
