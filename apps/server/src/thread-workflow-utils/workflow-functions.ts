...
import { getWhyWireAgent, getWhyWireSocketAgent, modifyThreadLabelsInDB } from '../lib/server-utils';
...
export const workflowFunctions: Record<string, WorkflowFunction> = {
  ...
  createDraft: async (context) => {
    ...
    const { stub: agent } = await getWhyWireAgent(context.connectionId);
    ...
    const socketAgent = await getWhyWireSocketAgent(context.connectionId);
