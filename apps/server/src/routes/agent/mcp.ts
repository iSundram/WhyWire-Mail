/*
 * Licensed to WhyWire Mail Inc. under one or more contributor license agreements.
 ...
 *
 * Reuse or distribution of this file requires a license from WhyWire Mail Inc.
 */

import { McpServer } from '@modelcontextprotocol/sdk/server/mcp.js';
import { getThread, getWhyWireAgent } from '../../lib/server-utils';
...
export class WhyWireMCP extends McpAgent<typeof env, Record<string, unknown>, { userId: string }> {
  server = new McpServer({
    name: 'whywire-mcp',
    version: '1.0.0',
    description: 'WhyWire MCP',
  });
  ...
  async init(): Promise<void> {
    ...
    const { stub: agent } = await getWhyWireAgent(_connection.id);
