...
import { connectionToDriver, getWhyWireSocketAgent, reSyncThread } from '../../lib/server-utils';
...
export interface TopicGenerationRequirements {
  ...
  readonly agent?: DurableObjectStub<WhyWireAgent>;
...
@Queryable()
export class WhyWireDriver extends DurableObject<ZeroEnv> {
  transfer = new Transfer(this);
  sql: SqlStorage;
  private db: DB;
  private syncThreadsInProgress: Map<string, boolean> = new Map();
  private driver: MailManager | null = null;
  private agent: DurableObjectStub<WhyWireAgent> | null = null;
  ...
  public async setupAuth() {
    ...
    if (!this.agent) this.agent = await getWhyWireSocketAgent(this.name);
  }
...
export class WhyWireAgent extends AIChatAgent<ZeroEnv> {
  private chatMessageAbortControllers: Map<string, AbortController> = new Map();

  async registerWhyWireMCP() {
